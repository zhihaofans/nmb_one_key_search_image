...
$(document).ready(function () {
	...
	if(localStorage.oksi_setting_autorun=="yes")
	{
		oksi_start();
	}
...
{
	if (localStorage.oksi_setting_search == undefined)
	{
		localStorage.oksi_setting_search = "baidu";
	}
	if (localStorage.oksi_setting_autorun == undefined)
	{
		localStorage.oksi_setting_autorun = "yes";
	}
	...
	case "sogou":
		...
		break;
	default:
		var oksi_select = "<select id=\"oksi_html_search\" name=\"oksi_html_search\" onchange=\"oksi_search_change(this.value)\"><option value=\"baidu\">baidu</option><option value=\"google\">google</option><option value=\"sogou\">sogou</option></select>";
	}
...
{
	localStorage.oksi_setting_search = _search;
	alert("设置完毕");
	location.reload();
}
function oksi_start()
{
	...
	var oksi_search_google="https://www.google.com.hk/searchbyimage?image_url=";
	switch(localStorage.oksi_setting_search)
	{
	case "baidu":
